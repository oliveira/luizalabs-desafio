...
test('should update email of path identification user', async (t) => {
  ...
  t.is(dataUpdate.email, bodyUpdate.email)
})
...
test('should return a 403 when trying to update another user data', async (t) => {
  ...
  const bodyUpdate = {
    email: 'bbbbbb@hello.com'
  }

  const responseUpdate = await fetch('http://localhost:3000/users/7777777', {
    method: 'PATCH',
    body: JSON.stringify(bodyUpdate),
    headers: {
      Accept: 'application/json',
